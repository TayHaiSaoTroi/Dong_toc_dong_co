...
#define ENABLE 8
Servo svo;
LiquidCrystal_I2C lcd(0x27,20,4);y
int s=0;
int T = 1;
...
lcd.init();
 ...
 Serial.begin(9600);
  ...
  svo.write(0);
  lcd.setCursor(1,0);
  lcd.print("TRUONG DH NONG LAM");
  lcd.setCursor(7,2);
  lcd.print("KHOA CK CN");
  lcd.setCursor(3,1);
...
void StepMotorRun(int xung, int t) 
{
  ...
  {
    digitalWrite(STEP, HIGH); // Output high - xung mức cao
    delay(t);
    digitalWrite(STEP, LOW);// Output low - xung mức thấp
    delay(t);
  }
}
